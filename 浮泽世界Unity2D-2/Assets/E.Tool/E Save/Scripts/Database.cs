//在SQLite数据库中保存字符数据。我们使用SQLite有几个原因
//
//  -  SQLite是基于文件的，无需设置数据库服务器即可运行
//  - 我们可以通过SQL查询轻松“删除所有...”或“全部修改...”
//  - 很多人都要求使用SQL数据库并且对XML不满意
//  - 我们可以允许各种角色名称，甚至是中文名称
//破坏文件系统
//  - 稍后使用多个服务器实例时，我们将需要MYSQL或类似的东西
//升级是微不足道的
//  -  XML更容易，但是：
//  - 我们不能轻易阅读'只是一个角色的类'等，但我们需要它
//经常用于角色选择等
//  - 如果每个帐户都是包含播放器的文件夹，那么我们无法保存
//除了我们使用的密码，禁止等其他帐户信息
//额外的account.xml文件，它使所有内容过于复杂
//  - 总会有像'COM'这样的禁止文件名，这会导致
//当人们尝试使用该名称创建帐户或字符时出现问题
//
//关于商品商城硬币：
//支付提供商的回调应该添加新订单
// character_orders表。然后服务器将在播放器处理它们
//是游戏。不要试图直接修改字符表中的“硬币”。
//
//打开sqlite数据库文件的工具：
...
//
//关于表现：
//  - 建议仅在使用SQLite连接时保持打开状态。
// MMO服务器一直使用它，所以我们一直保持打开状态。这也是
//允许我们轻松使用事务，并且它将转换为
// MYSQL更容易。
//  - 交易肯定是必要的：
//无需交易即可节省100名玩家需要3.6秒
//用交易保存100名玩家需要0.38秒
//  - 使用tr = conn.BeginTransaction（）+ tr.Commit（）并将其传递给所有人
//这些功能非常复杂。我们使用BEGIN + END查询代替。
//
//一些基准：
//保存100名未优化的玩家：4s
//保存100名玩家总是打开连接+交易：3.6s
//保存100名玩家总是打开连接+交易+ WAL：3.6s
//使用tr = ...'事务在1'中保存100名玩家：380ms
//在1个BEGIN / END样式交易中保存100个玩家：380毫秒
//用XML保存100名玩家：369ms
//
//构建笔记：
//  - 要求将播放器设置设置为“.NET”而不是“.NET Subset”，否则System.Data.dll导致ArgumentException。
//  - 需要独立的sqlite3.dll x86和x64版本（windows / mac / linux）在sqlite.org网站上找到
//  - 需要libsqlite3.so x86和armeabi-v7a for android使用android ndk r9b linux从sqlite.org amalgamation源编译
using UnityEngine;
using System;
using System.IO;
using System.Linq;
...
namespace E.Tool
{
    public partial class Database : SingletonPattern<Database>
    {
        // file name
        ...
        void LoadSkills(Player player)
        {
            ...
            // -> this way any data changes in a prefab will be applied
