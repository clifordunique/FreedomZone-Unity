...
public partial class Player : Entity
{
        ...
        set
        {
            ...
            else
            {
                ...
                while (experience >= ExperienceMax && level < maxLevel)
                {
                    ...
                    // addon system hooks
                    Utility.InvokeMany(typeof(Player), this, "OnLevelUp_");
                }
    ...
    protected override void Awake()
    {
        ...
        // addon system hooks
        Utility.InvokeMany(typeof(Player), this, "Awake_");
    }
    protected override void Start()
    {
        ...
        // addon system hooks
        Utility.InvokeMany(typeof(Player), this, "Start_");
    }
    private void LateUpdate()
    {
        ...
        // addon system hooks
        Utility.InvokeMany(typeof(Player), this, "LateUpdate_");
    }
    private void OnDestroy()
    {
        ...
        // addon system hooks
        Utility.InvokeMany(typeof(Player), this, "OnDestroy_");
    }
    ...
    public override void OnStartLocalPlayer()
    {
        ...
        // addon system hooks
        Utility.InvokeMany(typeof(Player), this, "OnStartLocalPlayer_");
    }
    ...
    public override void OnStartServer()
    {
        ...
        // addon system hooks
        Utility.InvokeMany(typeof(Player), this, "OnStartServer_");
    }
    ...
    protected override void UpdateClient()
    {
            ...
            if (isLocalPlayer)
            {
                ...
                if (useSkillWhenCloser != -1)
                {
                    ...
                    if (CanAttack(Target))
                    {
                        ...
                        if (Utility.ClosestDistance(collider, Target.collider) <= range)
                        {
        ...
        // addon system hooks
        Utility.InvokeMany(typeof(Player), this, "UpdateClient_");
    }
    ...
    public override void DealDamageAt(Entity entity, int amount, float stunChance = 0, float stunTime = 0)
    {
        ...
        // addon system hooks
        Utility.InvokeMany(typeof(Player), this, "DealDamageAt_", entity, amount);
    }
    ...
    protected override void OnDeath()
    {
        ...
        // addon system hooks
        Utility.InvokeMany(typeof(Player), this, "OnDeath_");
    }
    ...
    public void CmdTakeLootGold()
    {
        ...
        if ((State == "IDLE" || State == "MOVING" || State == "CASTING") &&
            ...
            Utility.ClosestDistance(collider, Target.collider) <= interactionRange)
        {
    ...
    public bool CanStartTradeWith(Entity entity)
    {
        ...
        return entity != null && entity is Player && entity != this &&
               ...
               Utility.ClosestDistance(collider, entity.collider) <= interactionRange;
    }
    ...
    void SelectionHandling()
    {
        ...
        if (Input.GetMouseButtonDown(0) && !Utility.IsCursorOverUserInterface() && Input.touchCount <= 1)
        {
            ...
            RaycastHit2D hit = localPlayerClickThrough ? Utility.Raycast2DWithout(ray, gameObject) : Physics2D.GetRayIntersection(ray);
            ...
            if (entity)
            {
                ...
                if (entity == Target && entity != this && entity != ActivePet)
                {
                    ...
                    // use collider point(s) to also work with big entities
                    else if (entity is Npc && entity.Health > 0 && Utility.ClosestDistance(collider, entity.collider) <= interactionRange)
                    {
                        UINpcDialogue.singleton.Show();
                    }
                    // monster, dead, has loot, close enough? => loot
                    // use collider point(s) to also work with big entities
                    else if (entity is Monster && entity.Health == 0 && Utility.ClosestDistance(collider, entity.collider) <= interactionRange && ((Monster)entity).HasLoot())
                    {
                        UILoot.singleton.Show();
                    }
                    else if (entity is ItemEntity && entity.Health >= 0 && Utility.ClosestDistance(collider, entity.collider) <= interactionRange)
                    {
                    ...
                    // addon system hooks
                    Utility.InvokeMany(typeof(Player), this, "OnSelect_", entity);
        ...
        //如果点击鼠标右键
        if (Input.GetMouseButtonDown(1) && !Utility.IsCursorOverUserInterface() && Input.touchCount <= 1)
        {
            Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
            RaycastHit2D hit = localPlayerClickThrough ? Utility.Raycast2DWithout(ray, gameObject) : Physics2D.GetRayIntersection(ray);
            useSkillWhenCloser = -1;
            ...
            if (entity)
            {
                if (entity == Target && entity != this && entity != ActivePet)
                {
                    if (entity is Npc && entity.Health > 0 && Utility.ClosestDistance(collider, entity.collider) <= interactionRange)
                    {
                        //UINpcDialogue.singleton.Show();
                    }
                    else if (entity is Monster && entity.Health == 0 && Utility.ClosestDistance(collider, entity.collider) <= interactionRange && ((Monster)entity).HasLoot())
    ...
    void OnDragAndDrop_InventorySlot_InventorySlot(int[] slotIndices)
    {
        ...
        // split?
        else if (Utility.AnyKeyPressed(inventorySplitKeys))
        {
